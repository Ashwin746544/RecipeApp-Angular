...
user = new BehaviorSubject<User>(null); // this is same as Subject but additionally we can get immediate value
...
login(email: string,password: string){
 ...
 autoLogin(){
   ...
   if(loadedUser.token){ // here we called getter token method
...
private handleError(errorRes: HttpErrorResponse){
  let errorMessage = 'An error Occured!';
      if(!errorRes.error || !errorRes.error.error){
        errorMessage = 'Please Check Your Network connention!';
      ...
      switch(errorRes.error.error.message){
        ...
        case 'EMAIL_NOT_FOUND': errorMessage = 'This email does not Exist!';
                                 break;
        case 'INVALID_PASSWORD': errorMessage = 'You entered wrong password!';
                                 break;
      }
     return throwError(errorMessage);
}
